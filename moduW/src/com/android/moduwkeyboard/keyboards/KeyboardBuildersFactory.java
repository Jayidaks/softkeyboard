...
public class KeyboardBuildersFactory {
    ...
    public synchronized static ArrayList<KeyboardBuilder> getAllBuilders(final Context context) {
        ...
        if (ms_creators == null)
        {
            ...
            keyboards.addAll(getKeyboardCreatorsFromResId(context, com.android.moduwkeyboard.R.xml.keyboards));
        ...
        ms_creators = sortKeyboardsByPriority(ms_creators);

        return ms_creators;
    }

    private static ArrayList<KeyboardBuilder> sortKeyboardsByPriority(
			ArrayList<KeyboardBuilder> keyboards) {
    	ArrayList<KeyboardBuilder> newKeyboards = new ArrayList<KeyboardBuilder>();
    	
    	//Hebrew first: keyboard_8958fb12-6558-4e96-9aa6-0e90101570b3
    	for(int i=0;i<keyboards.size();)
    	{
    		KeyboardBuilder aKeyboard = keyboards.get(i);
    		if (aKeyboard.getId().equals("keyboard_aa58fb12-6558-4e96-9aa6-0e90101570b3"))
    		{
    			newKeyboards.add(aKeyboard);
    			keyboards.remove(i);
    		}
    		else
    		{
    			//incing ONLY if a priority keyboard was found
    			i++;
    		}
    	}
    	//adding all the rest
    	newKeyboards.addAll(keyboards);
...
//                        }
                        ...
                        final int iconResId = attrs.getAttributeResourceValue(null,
                                ...
                                com.android.moduwkeyboard.R.drawable.sym_keyboard_notification_icon);
