...
public class AnySoftKeyboard extends InputMethodService implements
    ...
    public View onCreateInputView() {
        ThemeResources themeResources = ThemeSwitcher.getThemeResources(getApplicationContext());
        AttributeSet attrs = Xml.asAttributeSet(getResources().getXml(
                Workarounds.isDonut() ? R.layout.input_donut : R.layout.input_cupcake));

        mInputView = new AnyKeyboardView(getApplicationContext(), themeResources, attrs);
    ...
    public View onCreateCandidatesView() {
        ...
        mCandidateViewContainer = (CandidateViewContainer) inflate.inflate(themeResources
                .getCandidateViewContainerLayoutResourceID(), null);
        ...
        if (!restarting) {
            switch (attribute.inputType & EditorInfo.TYPE_MASK_CLASS) {
                ...
                case EditorInfo.TYPE_CLASS_TEXT:
                    ...
                    // startPrediction();
                    mPredictionOn = true;
                    // Make sure that passwords are not displayed in candidate
                    // view
                    ...
                    if ((!AnySoftKeyboardConfiguration.getInstance()
                            .getInsertSpaceAfterCandidatePick())
                            || // some users want to never get spaces added
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (DEBUG) {
            Log.d(TAG, "Event: Key:" + event.getKeyCode() + " Shift:"
                    + ((event.getMetaState() & KeyEvent.META_SHIFT_ON) != 0) + " ALT:"
                    + ((event.getMetaState() & KeyEvent.META_ALT_ON) != 0) + " Repeats:"
        ...
        switch (keyCode) {
            ...
            // onKey(Keyboard.KEYCODE_DELETE, new
            // int[]{Keyboard.KEYCODE_DELETE});
            // return true;
            ...
            default:
                ...
                if (!AnySoftKeyboardConfiguration.getInstance().getUseRepeatingKeys()
                        ...
                        if (event.isPrintingKey()) {
                            ...
                            if (DEBUG) {
                                ...
                                Log.v(TAG, "Hard Keyboard Action before translation: Shift: "
                                        + mHardKeyboardAction.isShiftActive() + ", Alt: "
                                        + mHardKeyboardAction.isAltActive() + ", Key code: "
                                        + mHardKeyboardAction.getKeyCode() + ", changed: "
    ...
    private void notifyKeyboardChangeIfNeeded() {
        ...
        if ((mKeyboardSwitcher.isAlphabetMode())
            ...
            notification.setLatestEventInfo(getApplicationContext(), getText(R.string.ime_name),
                    keyboardName, contentIntent);
            // this will not work. Need to find a way to show notification as a
            // different package.
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_DPAD_RIGHT:
                ...
                if (mInputView != null && mInputView.isShown() && mInputView.isShifted()) {
                    event = new KeyEvent(event.getDownTime(), event.getEventTime(), event
                            .getAction(), event.getKeyCode(), event.getRepeatCount(), event
                            .getDeviceId(), event.getScanCode(), KeyEvent.META_SHIFT_LEFT_ON
    ...
    private String getMetaKeysStates(String place) {
        ...
        return "Meta keys state at " + place + "- SHIFT:" + shiftState + ", ALT:" + altState
                + " SYM:" + symState + " bits:" + MyMetaKeyKeyListener.getMetaState(mMetaState)
                + " state:" + mMetaState;
    }
        ...
        if (mKeyboardSwitcher.isAlphabetMode()) {
            ...
            if (!currentKeyboard.isShifted()) {
                ...
                if (currentKeyboard.isShiftLocked()) {
                    ...
                    if (DEBUG) {
                        Log.d(TAG, "handleShift: current keyboard is shifted");
                    }
                    mInputView.setShifted(true);
                    caps = true;
                }
    ...
    private void handleCharacter(int primaryCode, int[] keyCodes) {
        if (DEBUG) {
            Log.d("AnySoftKeyboard", "handleCharacter: " + primaryCode + ", isPredictionOn:"
    ...
    private void handleSeparator(int primaryCode) {
        ...
        if (mPredicting) {
            ...
            if (mAutoCorrectOn
                    && primaryCode != '\''
                    && (mJustRevertedSeparator == null || mJustRevertedSeparator.length() == 0 || mJustRevertedSeparator
    ...
    private void updateSuggestions() {
        if (DEBUG) {
            Log.d("AnySoftKeyboard", "updateSuggestions: has mSuggest:" + (mSuggest != null)
                    + ", isPredictionOn:" + isPredictionOn() + ", mPredicting:" + mPredicting
            ...
            if (manager.getRingerMode() == AudioManager.RINGER_MODE_NORMAL) {
                ...
                // are not used
                {
                    ...
                    if (mSoundVolume < 0) {
                        ...
                        if (DEBUG) {
                            Log.d("AnySoftKeyboard",
                                    "Sound on key-pressed. Taking STREAM_NOTIFICATION: " + volume
                                            + " out of " + maxVolume);
                        }
                    } else {
                        volume = mSoundVolume;
                        maxVolume = 100;
                        if (DEBUG) {
                            Log.d("AnySoftKeyboard", "Sound on key-pressed. Taking custom volume: "
                    ...
                    if (Workarounds.isEclair()) {
                        ...
                        fxVolume = 8 * ((float) volume) / (maxVolume);
                    }
                }

                if (DEBUG) {
                    Log.d("AnySoftKeyboard", "Sound on key-pressed. Sound ID:" + keyFX
    ...
    private boolean loadSettings() {
        ...
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(this);
        int newVibrationDuration = Integer.parseInt(sp.getString("vibrate_on_key_press_duration",
                "0"));
        ...
        String newKeyboardChangeNotificationType = sp.getString(
                "physical_keyboard_change_notification_type", KEYBOARD_NOTIFICATION_ON_PHYSICAL);
    ...
    private Dictionary getDictionaryForKeyboard(AnyKeyboard currentKeyboard) {
        ...
        if (dictionaryValue == null) {
            dictionary = DictionaryFactory.getDictionaryByLanguage(currentKeyboard
                    .getDefaultDictionaryLocale(), this);
        } else {
            Log.d("AnySoftKeyboard", "Default dictionary '"
                    + (defaultDictionary == null ? "None" : defaultDictionary) + "' for keyboard '"
                    + currentKeyboard.getKeyboardPrefId() + "' has been overriden to '"
                    + dictionaryValue + "'");
    ...
    private String getDictionaryOverrideKey(AnyKeyboard currentKeyboard) {
        String mappingSettingsKey = currentKeyboard.getKeyboardPrefId() + "_override_dictionary";
                ...
                switch (position) {
                    ...
                    default:
                        if ((position < 0) || (position >= items.length)) {
                            ...
                            Log.d("AnySoftKeyboard", "Dictionary override. User selected "
    ...
    private void showOptionsMenu() {
        ...
        builder.setItems(new CharSequence[] {
                itemSettings, itemOverrideDictionary, itemInputMethod
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        Log.i("AnySoftKeyboard", "** Orientation:" + newConfig.orientation + " previous:"
                + mOrientation);
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        ...
        if (!handled) {
            /* AnyKeyboard removedKeyboard = */mKeyboardSwitcher.makeKeyboards(true);// maybe
            // a
            // new
            // keyboard
            /*
    ...
    public void appendCharactersToInput(CharSequence textToCommit) {
        if (DEBUG) {
            Log.d("AnySoftKeyboard", "appendCharactersToInput: " + textToCommit);
        }
    ...
    public void deleteLastCharactersFromInput(int countToDelete) {
        if (DEBUG) {
            Log.d("AnySoftKeyboard", "deleteLastCharactersFromInput: " + countToDelete);
        }
        if (countToDelete == 0) {
            return;
        }
