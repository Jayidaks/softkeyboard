...
public class ThemeResources {
    ...
    private void parseResMappingFromXml(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // We do not need to store reference to context if we don't have any resource identifiers that
            // would get inflated.
    ...
    public Context getPopupLayoutContext() {
        ...
        if (mContext != null) {
            if(mPopupLayoutResourceID == null){
            	// There is no PopupLayout defined here so we must fallback for Context, too
            	fallback = true;
            }else{
            	fallback = false;
            }
