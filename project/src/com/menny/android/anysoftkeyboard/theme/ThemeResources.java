...
public class ThemeResources {
    ...
    private static String XML_KEYBOARD_THEME_TAG = "AnySoftKeyboardTheme";
    ...
    private void parseResMappingFromXml(Context context, XmlResourceParser parser) {
        ...
        try {
            ...
            while ((event = parser.next()) != XmlPullParser.END_DOCUMENT) {
                ...
                if (event == XmlPullParser.START_TAG) {
                    ...
                    if (XML_KEYBOARD_THEME_TAG.equals(tag)) {
                        // inMappings = true;
                        if (AnySoftKeyboardConfiguration.getInstance().getDEBUG()) {
                            Log.d(TAG, "Starting parsing " + XML_KEYBOARD_THEME_TAG);
                        }
        ...
        if (!foundElement) {
            Log.w(TAG, "Could not find the " + XML_KEYBOARD_THEME_TAG + " element!");
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if ("keyBackground".equals(name)) {
                ...
                if (msThemeDrawableAttributeNames == null) {
                    Log.e(TAG,
                            "Something wrong with initialization (msThemeDrawableAttributeNames)!");
                } else {
                ...
                if (msThemeDimensionsAttributeNames == null) {
                    Log
                            .e(TAG,
                                    "Something wrong with initialization (msThemeDimensionsAttributeNames)!");
                } else {
                    ...
                    for (int j = 0; j < attributeCounts; j++) {
                        ...
                        if (attribute.equals(name)) {
                            mDimensionsMapping.put(id, res.getDimension(referenceId));
                            processed = true;
                            break;
                        }
                    }
                ...
                if (msThemeTypefacesAttributeNames == null) {
                    Log
                            .e(TAG,
                                    "Something wrong with initialization (msThemeTypefacesAttributeNames)!");
                } else {
    ...
    public static void initAttributeInfos(Context askContext) {
        if (askContext != null && msThemeDrawableAttributeNames == null) {
            msThemeDrawableAttributeNames = askContext.getResources().getStringArray(
                    R.array.theme_drawables_attribute_names);
            msThemeDrawableAttributeValues = Utils.getIntArrayOfReferences(askContext
                    .getResources(), R.array.theme_drawables_ids);
            msThemeColorsAttributeNames = askContext.getResources().getStringArray(
                    R.array.theme_colors_attribute_names);
            msThemeColorsAttributeValues = Utils.getIntArrayOfReferences(askContext.getResources(),
                    R.array.theme_colors_ids);
            msThemeDimensionsAttributeNames = askContext.getResources().getStringArray(
                    R.array.theme_dimensions_attribute_names);
            msThemeDimensionsAttributeValues = Utils.getIntArrayOfReferences(askContext
                    .getResources(), R.array.theme_dimensions_ids);
            msThemeTypefacesAttributeNames = askContext.getResources().getStringArray(
                    R.array.theme_font_typefaces_attribute_names);
            msThemeTypefacesAttributeValues = Utils.getIntArrayOfReferences(askContext
                    .getResources(), R.array.theme_font_typefaces_ids);
        }
    }
     ...
     */
    public ThemeResources(Context context, ThemeResourcesBuilder pluginInfo,
        ...
        try {
            externalContext = context.createPackageContext(pluginInfo.mPackageName, 0);
        } catch (NameNotFoundException e) {
            e.printStackTrace();
        }

        XmlResourceParser parser = externalContext.getResources().getXml(
                pluginInfo.mThemeDefinitionResId);
        mContext = externalContext;

        parseResMappingFromXml(externalContext, parser);
    }
     ...
     * @param defaultRes
     */
    public ThemeResources(Context context, XmlResourceParser parser, ThemeResources defaultRes) {
     ...
     * @param defaultRes
     */
    public ThemeResources(Context context, AttributeSet attrs, ThemeResources defaultRes) {
        mDefaultRes = defaultRes;
        mContext = context;
        mDrawablesMapping = new HashMap<Integer, Integer>();
        mColorsMapping = new HashMap<Integer, Integer>();
    ...
    public Typeface getTypeface(int id) {
        ...
        if (pathToTypeface != null) {
            ...
            if (assets == null) {
                Log.e(TAG, "Could not return Typeface since we have not Assets!");
                return Typeface.DEFAULT;
            }
            Typeface typeface = Typeface.createFromAsset(assets, pathToTypeface);
            if (typeface == null) {
    ...
    public Context getCandidatePreviewLayoutContext() {
        ...
        if (fallback && mDefaultRes != null) {
            ...
            Log.w(TAG, "Could not find suitable CandidatePreviewLayout Context.");
            return null;
        }
    }
    ...
    public Integer getCandidateViewContainerLayoutResourceID() {
        if (mCandidateViewContainerLayoutResourceID != null) {
            ...
            Log.w(TAG, "Could not find suitable CandidateViewContainerLayoutResourceID.");
            return 0;
        }
    }
    ...
    public Context getCandidateViewContainerLayoutContext() {
        ...
        if (fallback && mDefaultRes != null) {
            ...
            Log.w(TAG, "Could not find suitable CandidateViewContainerLayout Context.");
            return null;
        }
    }
